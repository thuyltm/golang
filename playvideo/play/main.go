...
func readVideoAndAudio(media *reisen.Media) (<-chan *image.RGBA, <-chan [2]float64,
	chan error, error) {
...
// streamSamples creates a new custom streamer
// for playing audio samples provided by the sourc channel
...
// Start reading samples and frames
// of the media file
func (game *Game) Start(fname string) error {
	...
	game.frameBuffer, sampleSource, game.errs, err =
		readVideoAndAudio(media)
	if err != nil {
		return err
	}
...
func (game *Game) Update(screen *ebiten.Image) error {
	...
	select {
	case <-game.perSecond:
		ebiten.SetWindowTitle(fmt.Sprintf("%s | FPS: %d | "+
			"dt: %f | Frames: %d | Video FPS: %d",
			"Video", game.fps, game.deltaTime, game.videoTotalFramesPlayed,
			game.videoPlaybackFPS))
		game.fps = 0
		game.videoPlaybackFPS = 0
	default:
...
func main() {
	game := &Game{}
	err := game.Start("/home/thuy/Documents/Learn/golang/playvideo/src/demo.mp4")
	handleError(err)
